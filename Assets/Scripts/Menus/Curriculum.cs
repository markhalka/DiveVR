...
public class Curriculum : MonoBehaviour
{
    ...
    void createNewLearningPlan()
    {
        Debug.LogError("creating a new learning plna...");
        ...
        foreach (var grade in Information.xmlDoc.Descendants("grade"))
        {
            if ("Grade " + grade.Attribute("number").Value == Information.grade)
            {
                foreach (var subject in grade.Elements())
                {
                    ...
                    {
                        Debug.LogError("found grade and subject");
                        subject.RemoveAll();
                        currLesson = subject;
                        break;
                    }
        ...
        for (int i = 0; i < Information.placmentScore.Count; i++)
        {
            ...
            if (isScience)
            {
                if (isIncluded)
                {
                    Debug.LogError("adding: " + Information.placmentTest[i].name + " to science file");
           ...
           new XAttribute("topics", Information.placmentTest[i].topics[0]));
                    currLesson.Add(newLesson);
                }
                else
                {
                    Debug.LogError("not adding: " + Information.placmentTest[i].name + " to science file");
                }
            }
            else
            {
    ...
    void createCurrent()
    {
        ...
        for (int i = 0; i < Information.topics.Count; i++)
        {
            ...
            //this will add all the current ones             
                ...
                if (currDefualt.name == Information.topics[i].name)
                {
                    ...
                    for (int j = 0; j < currDefualt.topics.Count; j++)
                    {
                        if (!currTopics.Contains(currDefualt.topics[j]))
                        {
                            ...
                            newValue.transform.SetParent(newValue.transform.parent.parent);
                            newValue.transform.GetComponent<Image>().color = currColor;
                            newValue.transform.GetComponentInChildren<TMP_Text>().text = names[currDefualt.topics[j]];
                            newValue.gameObject.SetActive(true);


                            userLessons.Add(newValue, new Lesson(currDefualt, j, false));
        ...
        foreach (var leftOver in currGradeDefualt)
        {
            if (!addedNames.Contains(leftOver.name))
            {
                ...
                for (int i = 0; i < leftOver.topics.Count; i++)
                {
                    ...
                    newValue.transform.SetParent(newValue.transform.parent.parent);
                    newValue.transform.GetComponent<Image>().color = currColor;
                    newValue.transform.GetComponentInChildren<TMP_Text>().text = names[leftOver.topics[i]]; //this will get the current name
                    newValue.gameObject.SetActive(true);


                    userLessons.Add(newValue, new Lesson(leftOver, i, false)); //not added
                }
            }
        }
    }
    ...
    void takeRedo()
    {
        ...
        Debug.LogError(Information.placmentScore.Count + " placment score count");
    ...
    int getTopicFromName(string name)
    {

        for (int i = 0; i < Information.topics.Count; i++)
        {
            if (Information.topics[i].name == name)
            {
                return i;
            }
        }
